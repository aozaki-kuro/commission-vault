...
- **GitHub Actions cache:** `ci` / `deploy` / `rebuild` restore `.turbo/`, and deploy/rebuild also restore/save the full `apps/web/generated/*` tree so Turbo replay and fact-source reuse can happen in the same job.
