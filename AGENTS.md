...
- **Cloudflare deploy commands:** manual repo-level deploy entrypoints are `bun run deploy:web` and `bun run deploy:admin`, which delegate into each workspace-local `wrangler.jsonc`. GitHub Actions admin deploy must use `apps/admin-worker/wrangler.ci.jsonc` plus the validated `apps/admin/dist` artifact so CI does not pay a second admin build through Wrangler's custom build hook. Do not restore a repo-root `wrangler.jsonc`; Cloudflare Workers Builds must connect the same repo to two Workers with root directories `apps/web` and `apps/admin-worker`, because push-triggered builds are resolved from Dashboard root/build/deploy settings, not from a shared repo-root config.
- **GitHub Actions cache:** `ci` / `deploy` / `rebuild` restore `.turbo/`, and deploy/rebuild also restore/save the full `apps/web/generated/*` tree so Turbo replay and fact-source reuse can happen in the same job. `deploy` must wait for a successful `CI` `workflow_run` on `master` instead of racing the same push, and release workflows must share both one `release-master` Turbo cache namespace and one concurrency group so they never reserve the same exact cache key at the same time.
- **Deploy workflow rule:** repo-level web deploy/rebuild workflows should not pre-run `web:fact-source:export` or `build:web` before `wrangler deploy`; the web workspace-local `wrangler.jsonc` custom build already owns those steps. Admin CI is the deliberate exception: `validate` builds `apps/admin/dist` once, uploads it as an artifact, and the deploy job reuses that artifact through `apps/admin-worker/wrangler.ci.jsonc` so admin is not rebuilt inside Wrangler.
- **Turbo cache safety:** `apps/web` build depends on remote D1/R2 data as well as source code, so any Turbo-backed web build in CI must include an explicit `WEB_BUILD_CACHE_TOKEN` invalidation input. Never let web deploy/rebuild read Turbo cache from source hashes alone.
...
- Added `apps/admin-worker/wrangler.ci.jsonc` and switched `.github/workflows/ci.yml` admin deploy to reuse the validated `apps/admin/dist` artifact, removing the previous double invocation of `build:admin` across validate + deploy.
...
- Changed repo-level deploy orchestration to trigger from successful `CI` completion on `master`, moved release cache writes onto a shared `release-master` Turbo namespace, and serialized `deploy` / `rebuild` in one release concurrency lane so cache reservations no longer race across workflows.
- Routed `apps/web` / `apps/admin-worker` Wrangler custom builds back through repo-root Turbo entrypoints, restored `.turbo/` in deploy/rebuild workflows, and added `WEB_BUILD_CACHE_TOKEN` invalidation so GitHub Actions can reuse safe Turbo hits without serving stale remote-data builds.
