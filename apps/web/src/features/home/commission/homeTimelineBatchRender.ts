...
const TITLE_WRAPPER_CLASS = 'mb-2 pt-4'
...
function renderEntryImage(entry: HomeTimelineBatchEntryPayload) {
...
function renderInterestButton(entry: HomeTimelineBatchEntryPayload) {
...
function renderEntryInfo(entry: HomeTimelineBatchEntryPayload) {
...
function renderEntry(entry: HomeTimelineBatchEntryPayload) {
  const root = document.createElement('div')
  root.id = entry.id
