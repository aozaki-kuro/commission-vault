export const ANALYTICS_EVENTS = {
  searchUsed: 'search_used',
  popularKeywordClicked: 'popular_keyword_clicked',
  suggestionSelected: 'suggestion_selected',
  keywordRefreshClicked: 'keyword_refresh_clicked',
  shuffleClicked: 'shuffle_clicked',
  hamburgerMenuUsed: 'hamburger_menu_used',
  sidebarNavUsed: 'sidebar_nav_used',
  sidebarViewModeToggleUsed: 'sidebar_view_mode_toggle_used',
  iWantToSeeIt: 'i_want_to_see_it',
  commissionImageVariantLoaded: 'commission_image_variant_loaded',
} as const
